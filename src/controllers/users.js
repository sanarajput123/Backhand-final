...
const updateUser = async (req, res) => {
    try {
        ...
        const hashedPassword = newPassword ? bcrypt.hashSync(newPassword, 8) : user.password;
        ...
        const updateUser = await Users.updateOne({ email }, updateDocument, { new: true });

        if (updateUser) {
            return res.status(200).send(`${email} updated successfully. ${user}`);
